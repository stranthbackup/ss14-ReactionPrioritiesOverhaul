        ...
        for (var i = 0; i < chosenPacks.Length; i++)
        {
            ...
            foreach (var roomSize in pack.Rooms)
            {
                ...
                if (!roomProtos.TryGetValue(roomDimensions, out var roomProto))
                {
                    ...
                    {
                        Matrix3.MultiplyBackwards(packTransform, dungeonTransform, out matty);
                ...
                Matrix3.MultiplyBackwards(roomTransform, packTransform, out matty);
                Matrix3.MultiplyBackwards(matty, dungeonTransform, out var dungeonMatty);
